...
def handle_data():
    ...
    dataset.drop(['BusinessTravel', 'Attrition','EmployeeNumber','Atrition_Prediction_Value', 'Risk'],axis = 1, inplace=True)
    #TRANSFORM CATEGORICAL COLUMNS INTO DUMMY VARIABLES FOR THE MODEL TO WORK PROPERLY
    cols_to_transform = ['Department', 'EducationField', 'Gender', 'JobRole' , 'MaritalStatus' , 'Over18', 'OverTime']
    ...
    res = client.invoke_endpoint(
                    EndpointName='xgboost-2018-09-23-04-48-47-593',
    ...
    dataset.to_csv(input_file,header=True, index=False)
